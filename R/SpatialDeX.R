...
#' @export
full.anno <-  load(
    system.file("extdata", "full.anno.RData", package = "SpatialDeX"))

#' @title cyclegenes
#' @export
cyclegenes <-  load(
    system.file("extdata", "cyclegenes.RData", package = "SpatialDeX"))

#' @title DNA.hg20
#' @export
DNA.hg20 <-  load(
    system.file("extdata", "DNA.hg20.RData", package = "SpatialDeX"))
...
#' @param top_cell_type defind the number of cell types to retain based on the
#'    proportion of each cell type. Choose an integer from 0 to 9.
...
replace_smallest_with_zero <- function(row,top_cell_type) {
    smallest_indices <- order(row)[seq_len(9 - top_cell_type)]
...
#' @param top_cell_type defind the number of cell types to retain based on
#'     the proportion of each cell type. Choose an integer from 0 to 9.
#' @importFrom dplyr %>%
#' @importFrom dplyr filter
...
SpatialDeX <- function(raw_exprs,
    ...
    prediction_result<-data_feature%*%t(weighted_feature)%*%
      solve((weighted_feature %*%
               t(weighted_feature)) +lambda * diag(nrow(weighted_feature)))
    ...
    if(length(rm_spot)>0){
      prediction_result_rm <- data.frame(
        prediction_result[!rownames(prediction_result) %in% names(rm_spot), ]
      )

      uniden_spot <- data.frame(
        matrix(rep("unidentified", length(names(rm_spot)) * 9),
               nrow = length(names(rm_spot)))
      )
    ...
    if (!is.null(top_cell_type)) {
      prediction_result_filter <- as.data.frame(
        t(apply(prediction_result_rm, 1,
                replace_smallest_with_zero,
